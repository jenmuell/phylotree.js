...
(function () {
  ...
  d3.layout.phylotree = function (container) {
    ...
    var self = {},
      ...
      //HERE IS A CHANGE:  change for labels of inner nodes
      branch_length_accessor = def_branch_length_accessor,
      def_node_label = function (_node) {
        if (options["internal-names"] || d3_phylotree_is_leafnode(_node) || d3_phylotree_is_node_collapsed(_node)) {
          return d3_phylotree_is_leafnode(_node) || d3_phylotree_is_node_collapsed(_node) ? _node.name : _node["show-name"] || "";
        }
        return "";
      },
    ...
    phylotree.placenodes = function () {
      ...
      function tree_layout(a_node) {
        ...
        if (is_leaf) {
          ...
          if (d3_phylotree_is_node_collapsed(a_node) && !is_under_collapsed_parent) {
            ...
            save_x = x;
            // save_span = last_span * 0.5;
            save_span = last_span === 0 ? 0.5 : last_span;
      ...
      function do_lr() {
        ...
        if (options["left-right-spacing"] == "fixed-step") {
          ...
          scales[1] =
            (size[1] - offsets[1] - options["left-offset"] - label_width) / _extents[1][1];
        }
      }
      ...
      if (phylotree.radial()) {
        ...
        var compute_distance = function (r1, r2, a1, a2, annular_shift) {
          ...
          return Math.sqrt(
            (r2 - r1) * (r2 - r1) +
              2 * (r1 + annular_shift) * (r2 + annular_shift) * (1 - Math.cos(a1 - a2))
          );
        };
        ...
        nodes.forEach(function (d) {
          if (!d.children) {
            ...
            if (last_child_angle !== null) {
              ...
              var local_mr =
                radial_dist > 0 ? required_spacing / radial_dist : 10 * options["max-radius"];
              ...
              if (local_mr > options["max-radius"]) {
                ...
                var dd = required_spacing / options["max-radius"],
                  ...
                  c =
                    ...
                    (dd * dd - (last_child_radius - d.radius) * (last_child_radius - d.radius)) /
                      2 /
        ...
        if (at_least_one_dimension_fixed) {
          radius = Math.min(
            radius,
            (Math.min(effective_span, _extents[1][1] * scales[1]) - label_width) * 0.5 -
        ...
        nodes.forEach(function (d) {
          ...
          if (d.collapsed) {
            ...
            d.collapsed = d.collapsed.filter(function (p, i) {
              ...
              if (
                Math.sqrt(Math.pow(p[0] - last_point[0], 2) + Math.pow(p[1] - last_point[1], 2)) > 3
              ) {
                last_point = p;
                return true;
              }
      ...
      if (draw_scale_bar) {
        ...
        if (phylotree.radial()) {
          ...
          domain_limit = Math.pow(
            10,
            Math.ceil(Math.log((_extents[1][1] * range_limit) / radius) / Math.log(10))
          );
        ...
        if (phylotree.radial()) {
          ...
          draw_scale_bar.ticks(
            Math.min(
              10,
              d3.round(
                range_limit / (shown_font_size * scaleTickFormatter(my_ticks).length * 0.8),
                0
              )
            )
          );
    ...
    phylotree.pad_height = function () {
      if (draw_scale_bar) {
       ...
       return 0;
      //return padding_top * phylo_attr[0];
    };
        ...
        if (!d3_phylotree_is_leafnode(node)) {
          if (options["collapsible"]) {
            ...
            if (options["selectable"]) {
              ...
              menu_object.append("li").attr("class", "dropdown-header").text("Selection");
            }
          }
          ...
          if (options["selectable"]) {
            ...
            if (
              phylotree
                .select_all_descendants(node, true, true)
                .reduce((acc, node) => !node[selection_attribute_name] || acc, false)
            ) {
              menu_object
                .append("li")
                .append("a")
                .attr("tabindex", "-1")
                .text("Select all descendant branches")
                .on("click", function (d) {
                  menu_object.style("display", "none");
            ...
            if (
              ...
              menu_object
                ...
                .append("a")
                .attr("tabindex", "-1")
                .text("Unselect all descendant branches")
                .on("click", function (d) {
                  menu_object.style("display", "none");
                  phylotree.modify_selection(
                    phylotree.select_all_descendants(node, true, true),
        ...
        if (node.parent) {
          if (options["selectable"]) {
            ...
            menu_object
              ...
              .append("a")
              .attr("tabindex", "-1")
              .text(`${node[selection_attribute_name] ? "Unselect" : "Select"} indicent branch`)
              .on("click", function (d) {
                menu_object.style("display", "none");
    ...
    phylotree.sync_edge_labels = function () {
      ...
      if (phylotree.count_handler()) {
        ...
        d3_phylotree_trigger_count_update(phylotree, counts, phylotree.count_handler());
      }
    };
    ...
    phylotree.internal_label = function (callback, respect_existing) {
      ...
      for (var i = nodes.length - 1; i >= 0; i--) {
        var d = nodes[i];
        if (
          !(d3_phylotree_is_leafnode(d) || d3_phylotree_item_selected(d, selection_attribute_name))
        ) {
          ...
          if (delete_me_idx >= 0) {
            ...
            if (node.parent.children.length > 2) {
              ...
              if (node.parent.parent) {
                node.parent.parent.children[node.parent.parent.children.indexOf(node.parent)] =
    ...
    phylotree.update = function (transitions) {
      ...
      enclosure.enter().append("g").attr("class", css_classes["tree-container"]);
      ...
      if (transitions) {
        ...
        collapsed_clades.enter().insert("path", ":first-child");
        collapsed_clades.attr("class", css_classes["clade"]).attr("d", function (d) {
          return (d.collapsed_clade = spline(d.collapsed));
        });
      }
      ...
      // let test = nodes.filter(n=> !d3_phylotree_node_visible(n))
      // console.log(test)
    ...
    phylotree.draw_node = function (container, node, transitions) {
      container = d3.select(container);
      // HERE IS A CHANGE: change style of collapsed nodes
      if (d3_phylotree_is_leafnode(node) || d3_phylotree_is_node_collapsed(node)) {
          ...
          (transitions ? labels.transition() : labels)
            .attr("transform", function (d) {
              return (
                ...
                d3_phylotree_svg_translate(phylotree.align_tips() ? phylotree.shift_tip(d) : null)
              );
            })
        ...
        if (node["own-collapse"]) {
          ...
          labels.exit().remove(); // Remove the old ones
  ...
  function d3_phylotree_resize_svg(tree, svg, tr) {
    ...
    if (tree.radial()) {
      var pad_radius = tree.pad_width(),
        vertical_offset =
          tree.options()["top-bottom-spacing"] != "fit-to-size" ? tree.pad_height() : 0;

      sizes = [sizes[1] + 2 * pad_radius, sizes[0] + 2 * pad_radius + vertical_offset];
      ...
      sizes = [
        sizes[1] + (tree.options()["left-right-spacing"] != "fit-to-size" ? tree.pad_width() : 0),
  ...
  //d3.layout.phylotree.is_node_visible = d3_phylotree_node_visible;
  // d3.layout.phylotree.place_nodes = phylotree.placenodes;
